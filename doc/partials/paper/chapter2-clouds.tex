...
Clouds are a substantial part of Earth's weather. They provide shade from the glistening sun on hot days and reflect the ground heat at night, keeping the surface warmer.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=1.2]
        ...
        % thermal
        \node (t1) at (4, 0.5) {};
        \node[red,text width=4cm] (t1t) at (5.9, 1.2) {column of warm air rising upwards};
        \node (t2) at (4, 1.8) {};
        \draw[red,edge] (t1) -- (t2);
        ...
        \node (r1) at (5, 4.0) {};
        \node (r2) at (6, 3.0) {};
        \node[cyan,text width=4cm] (r2t) at (7.5, 3.8) {cooled air sinking \phantom{xx} \phantom{xx} downwards};
...
According to \emph{metoffice} \cite{metoffice:weatherfronts}, weather fronts are boundaries between two air masses. Those masses differ in temperature, wind direction and humidity.
...
which means that \gls{thermal}s are created and clouds start to form at the border of the two air masses \cite{ww2010:warmfront}.
...
A cold front represents the boundaries of an air mass carrying cold air. 
...
As colder air is denser than warmer air, it pushes underneath it.
...
There is also the phenomenon of front \gls{occlusion}, producing \emph{\gls{occludedfront}}.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.47\linewidth}
        ...
        \end{tikzpicture}
        \captionof{figure}{Cold \gls{occlusion}: cool air catches up with a preceding \gls{coldfront}, forcing the warmer air in-between to go up, creating a \gls{thermal}.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.47\linewidth}
        ...
        \end{tikzpicture}
        \captionof{figure}{Warm \gls{occlusion}: a \gls{coldfront} catches up with a \gls{warmfront} preceded by cool air, forcing the warmer air in-between to go up, creating a \gls{thermal}.}
...
% \begin{figure}[H]
%     \includegraphics[width=\linewidth]{cloudtypes.png}
%     \captionof{figure}{Distinct classifications of cloud shapes in the troposphere \protect\cite{cloudtypes:wiki}.}
...
\begin{figure}[H]
    \begin{tikzpicture}[scale=1]
        \tikzset{edge/.style = {-{Latex[length=3mm]},shorten >= -4pt}}
        \tikzset{shortedge/.style = {shorten <=-4pt,shorten >= -4pt}}
        \tikzset{small/.style = {font=\small}}
        \tikzset{mini/.style = {font=\footnotesize}}
        \tikzset{tiny/.style = {font=\tiny}}

        \node[right,inner sep=0pt] at (-0.1,0)
            {\includegraphics[width=\textwidth]{cloudtypes.png}};

        % cloud layer names
        \node[mini,right,darkdarkcyan] at (1.5,4.0) {Cumulonimbus};
        \node[mini,right,darkdarkcyan] at (5.8,2.55) {Cirrocumulus};
        \node[mini,right,darkdarkcyan] at (9,2.6) {Cirrostratus};
        \node[mini,right,darkdarkcyan] at (11.5,3.9) {Cirrus};
        \node[mini,right,darkdarkcyan] at (5.0,0.7) {Altocumulus};
        \node[mini,right,darkdarkcyan] at (8.3,0.8) {Altostratus};
        \node[mini,right,darkdarkcyan] at (12.1,1.4) {Nimbostratus};
        \node[mini,right,darkdarkcyan] at (5.7,-1.4) {Cumulus};
        \node[mini,right,darkdarkcyan] at (9.3,-0.85) {Stratocumulus};
        \node[mini,right,darkdarkcyan] at (8.2,-2.35) {Stratus};

        % low mid high
        \node[mini,darkdarkcyan] at (0.45,-2.5) {Low};
        \node[mini,darkdarkcyan] at (0.45,0) {Mid};
        \node[mini,darkdarkcyan] at (0.45,2.5) {High};

        % height numbers
        \node[tiny,darkcyan!60] at (0.45,-1.2) {2000m};
        \node[tiny,darkcyan!80] at (0.45,0.9) {7000m};
        \node[tiny,darkcyan] at (0.45,4.1) {11000m};
        
    \end{tikzpicture}
    \captionof{figure}{Distinct classifications of cloud shapes in the troposphere \protect\cite{cloudtypes:wiki}.}
    \label{img:tikz:cloudtypes} 
\end{figure}

\noindent
This graphic above provides and excellent overview of all distinct cloud types.
Each type is depicted in its signature shape and labeled with its scientific name.
...
This corresponds to the altitude at which the cloud usually forms, up to eleven kilometers above ground. 
\\
...
They are made up of many small, white, puffy clouds called \emph{\gls{cloudlet}}s. Their woolly look give the cloud the name suffix \emph{cumulus}.
\\
\noindent
Cirrocumulus clouds are relatively rare, as they are naturally only formed when a turbulent vertical current meets a cirrus cloud layer. The cirrus cloud then disperses into many \gls{cloudlet}s.
\emptyline
...
They are often described as "high fog" as they have similarities in appearance.
\\
...
Their top is often shaped like an anvil, whereas the base is flat and dark, giving them a menacing look.
\\
...
Since clouds are classified into three major layers, it is reasonable to create an implementation approach that follows a similar structure.
Knowing that there are only a few differences between some cloud types, they could be combined or grouped together.
The next section describes and illustrates a possible implementation approach with an architecture that respects the three cloud layers.