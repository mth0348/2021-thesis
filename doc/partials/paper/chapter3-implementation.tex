...
\\
Given a \gls{shader} is flexible enough to render altostratus clouds, then it is most likely also able to render altocumulus, with only few adjustments necessary.
...
one for Bern, one in-between both cities, using interpolated weather data, and one for the distant city.
\\
